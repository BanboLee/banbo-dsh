...
**未来实现子代理默认使用 `provider: deepseek`、`model: deepseek-v4-flash`；直接人类可以显式覆盖具体 invocation，覆盖事实必须在 routing manifest 中逐次记录。当前 Todo 3 continuation 已由直接人类覆盖为 `provider: runtime-default`（沿用主协调者路由）、`model: gpt-5.6-sol`。F1–F5 Round 1 后，直接人类先规定 fixer 使用 `runtime-default`/`gpt-5.6-sol`；T8-accept-fix-2 已按此完成，T8-accept-fix-3 被人类中断。T8-accept-fix-4 与 T8-accept-fix-5 按当时人类要求使用 `deepseek`/`deepseek-v4-pro`。自 F1–F5 Round 4 之后，直接人类最新规定所有后续 preflight/acceptance fixer 与最终 reviewer 均固定使用 `provider: runtime-default`、`model: gpt-5.5`，并明确禁止派生任何 subagent；不得再使用 `deepseek-v4-flash`、`deepseek-v4-pro` 或 `gpt-5.6-sol`。**
...
- routing 历史事实不可改写；reviewer response 使用固定 JSON envelope。F2 是上述审计接口的最终 verifier。Round 5 是直接人类强制终止且没有任何 verdict 被采用的异常轮次：该轮保留 `round-5.cancelled.json` 与 `human-decisions.jsonl` 的 `round5-routing-waiver`，不得事后补造 routing，并从最终验收轮次中排除。对已有完整 launch routing 但 provider/tool 失败而没有返回 verdict envelope 的 reviewer（例如 F4 Round 6/7），coordinator 必须原样持久化 `failed-no-result`，该不完整 round 整轮不得用于完成判定，也不得伪造 verdict；这类失败不需要重写 routing，但必须在后续重新运行完整 F1-F5。只有 routing 完整且五份 verdict 齐全的后续同轮五审可用于完成判定。
...
- coordinator 独占写入 append-only `.omo/evidence/dsh-lsp-diagnostics-v1/final/coordinator/subagent-routing.jsonl`。每次启动任何 Todo 1-7 implementation、Todo 6 early-owner repair、Todo 8/acceptance fixer 或 F1-F5 reviewer **之前**追加一行 strict JSON object：`{"invocationId":"<unique-stable-id>","identity":"<stable-id>","role":"implementation|integration-repair|fixer|reviewer","provider":"<actual-provider-route>","model":"<exact-model-id>","prompt":"<完整逐字prompt>","round":<positive-int>,"baseSha":"<40-hex>"}`；不得只记 prompt 摘要/hash，不得事后补造。Todo 1-7 implementation 默认 provider/model 为 `deepseek`/`deepseek-v4-flash`；直接人类显式覆盖的 invocation 按 routing manifest 记录并优先于默认值：T3-r2 与 T8-accept-fix-2 为 `runtime-default`/`gpt-5.6-sol`；T8-accept-fix-3 被人类中断；T8-accept-fix-4 与 T8-accept-fix-5 使用 `deepseek`/`deepseek-v4-pro`；自 T8-accept-fix-6 起所有 fixer 固定为 `runtime-default`/`gpt-5.5` 且禁止派生 subagent。F1-F5 Round 1–3 的历史 model 按原 routing 记录；自 Round 4 起 model 必须为 `gpt-5.5`，provider 字段记录启动时的实际 provider/route，不硬编码为 `deepseek`。identity 对 Todo 固定 `T1`…`T7`（Todo6 continuation仍为`T6`）、reviewer 固定 `F1`…`F5` 并跨重审复用；fixer identity 固定为其 round 名，`invocationId` 对每次调用唯一。JSONL 本身不含 secret，prompt 禁止嵌入 secret。
- coordinator 在每轮 response 到达后写对应既有 evidence response 文件，不改 routing 原行。F2 对所有已完成 invocation 将 routing 行与 commit/evidence/response 一一核对；对当前并行 F1-F5 round 则核对 coordinator 在启动批次前已写齐五行及其 identity/round/baseSha，当前五个 response 由 coordinator 收齐后校验 envelope 并供下一轮/最终审计。F2 对 Todo 1-7 implementation 默认断言 provider/model 精确为 `deepseek`/`deepseek-v4-flash`，且 prompt 逐字含对应 selector；对有直接人类显式覆盖的 invocation 改为核验 override 事实与 routing 行，T3-r2 与 T8-accept-fix-2 必须为 `runtime-default`/`gpt-5.6-sol`，T8-accept-fix-3 必须记录为被人类中断且无提交，T8-accept-fix-4 与 T8-accept-fix-5 必须为 `deepseek`/`deepseek-v4-pro`；自 T8-accept-fix-6 起 fixer 必须为 `runtime-default`/`gpt-5.5` 且 prompt 明确禁止派生 subagent；对 F1-F5 Round 1–3 按历史 routing 核验 `gpt-5.6-sol`；自 Round 4 起断言 model 精确为 `gpt-5.5`、provider 与实际启动 route 一致，且 prompt 逐字含 `model: gpt-5.5`。除 `round5-routing-waiver` 覆盖的 Round 5（仅保留取消证据且不得补造 routing/response）外，缺行、多行、错误模型或错误 round/base 均 REJECT；已完整启动但 provider/tool 无结果的 invocation 必须对应原样 `failed-no-result` 文件，并使该 round 整轮失效、后续完整重跑，不将其伪装为 APPROVE/REJECT。最终完成判定只能使用同一 round 内五条 routing 与五份 verdict envelope 均齐全且 round/baseSha/commitSha 一致的 F1-F5 审查轮次。
- 直接人类决策的唯一证据源为 `.omo/evidence/dsh-lsp-diagnostics-v1/final/coordinator/human-decisions.jsonl`；F2 必须把其中 `T3-model-override`、`preflight-peer-scope`、`post-review-fixer-model`、`round5-routing-waiver` 与对应 routing/commit/取消证据逐项关联，不得把人类覆盖误判为默认路由偏差。
- `T8-fix-1` 是直接人类已批准的有界 preflight peer-pin 例外：其 launch prompt 精确列出获批的五个实际修改路径、RED/GREEN/全量命令与禁止项，但发生在本计划后来固化“每个 fixer prompt 必须逐字携带完整 FIXER_OWNED_PATHS/全部行为契约”之前；F2 应按 `human-decisions.jsonl` 的 `preflight-peer-scope` 与实际五路径提交审计该历史事实，不得改写 routing 历史，也不得将该单次历史例外推广到后续 fixer。
...
- Todo6 early-owner repair 在无额外人类覆盖时沿用其 invocation 的 provider/model；T8-accept-fix-2 提示使用 `provider: runtime-default`、`model: gpt-5.6-sol`；T8-accept-fix-4 与 T8-accept-fix-5 提示使用 `provider: deepseek`、`model: deepseek-v4-pro`；自 T8-accept-fix-6 起任何 Todo8 integration fixer 或 acceptance fixer 提示必须逐字包含 `provider: runtime-default`、`model: gpt-5.5` 和“禁止派生 subagent”。
- 恰好创建五个 acceptance subagent identity：F1-F5；自 Round 4 起均使用 `model: gpt-5.5`，provider/route 使用启动时承载该模型的实际值并写入 routing manifest。修复后复用这五个 identity 重审，不新增第六个 reviewer；每位 reviewer 的 prompt 必须明确禁止派生 subagent。
...
- [x] 8. Wave 4：串行预验收与最终 SHA
  ...
  Recommended task executor category: unspecified-high
  Executor: coordinator；T8-accept-fix-2 使用 runtime-default/gpt-5.6-sol；T8-accept-fix-4/5 使用 deepseek/deepseek-v4-pro；自 T8-accept-fix-6 起 fixer 使用 runtime-default/gpt-5.5 且禁止派生 subagent

### 固定 fixer contract（Todo 8 preflight 与 F1-F5 后共用）

- 一轮只启动一个 fixer；T8-accept-fix-2 使用 `runtime-default`/`gpt-5.6-sol`，T8-accept-fix-3 被人类中断，T8-accept-fix-4/5 使用 `deepseek`/`deepseek-v4-pro`；自 T8-accept-fix-6 起固定 `provider=runtime-default`、`model=gpt-5.5`，且禁止派生任何 subagent。
...
- 恰好 F1-F5 五个 acceptance subagent，同轮并行；自 Round 4 起全部使用 `model=gpt-5.5`，provider/route 记录实际启动值。每位 reviewer 必须独立完成自己的专业审查，禁止再派生任何 subagent。
...
- Executor: `model=gpt-5.5`（provider/route 取实际启动值并记录），严格只读；禁止派生 subagent。
...
- 对每个 T1-T7、Todo6 back-repair、fixer 与每轮 F1-F5 invocation 一一校验 invocationId/identity/role/provider/model/完整prompt/round/baseSha、启动前 append 顺序、response与commit/evidence对应；特别断言 T1-T7 implementation 默认使用 `provider=deepseek`、`model=deepseek-v4-flash`，人类显式覆盖的 T3-r2 与 T8-accept-fix-2 使用 `runtime-default`/`gpt-5.6-sol`，T8-accept-fix-3 记录为中断无提交，T8-accept-fix-4/5 使用 `deepseek`/`deepseek-v4-pro`，自 T8-accept-fix-6 起 fixer 使用 `runtime-default`/`gpt-5.5` 且禁止派生；F1-F5 Round 1–3 按历史 routing 使用 `gpt-5.6-sol`，自 Round 4 起使用 `model=gpt-5.5` 且 provider/route 与实际启动值一致，并验证 F1-F5 identity 跨重审复用。若某轮 F1-F5 已有完整 launch routing 但个别 provider/tool 无 verdict，F2 只接受原样 `failed-no-result` 作为失败状态记录并判定该整轮失效，不得把它当作 reviewer verdict envelope 或最终验收输入。
- APPROVE要求每任务 evidence语法通过、stage路径落在精确ownership、Todo6 early-owner授权链完整、无动态文件授权、routing无缺失/补造、failed-no-result 轮次已被排除且后续完整重跑、coordinator-only repair合规且其后五角色全量重审。
...
- 实现 agent 默认使用 `provider=deepseek`、`model=deepseek-v4-flash`；直接人类显式覆盖的 T3-r2 使用 `provider=runtime-default`、`model=gpt-5.6-sol`。T8-accept-fix-2 使用 `runtime-default`/`gpt-5.6-sol`，T8-accept-fix-3 中断无提交，T8-accept-fix-4/5 使用 `deepseek`/`deepseek-v4-pro`；自 T8-accept-fix-6 起所有 integration/acceptance fixer 固定 `runtime-default`/`gpt-5.5` 且禁止派生 subagent。F1-F5 Round 1–3 reviewer 使用历史 `gpt-5.6-sol`；自 Round 4 起最终 reviewer 固定 `model=gpt-5.5`，provider/route 记录实际启动值，且禁止派生 subagent。
