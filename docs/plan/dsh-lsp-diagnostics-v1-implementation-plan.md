...
- routing 历史事实不可改写；reviewer response 使用固定 JSON envelope。F2 是上述审计接口的最终 verifier。Round 5 是直接人类强制终止且没有任何 verdict 被采用的异常轮次：该轮保留 `round-5.cancelled.json` 与 `human-decisions.jsonl` 的 `round5-routing-waiver`，不得事后补造 routing，并从最终验收轮次中排除；只有 routing 完整且五份 verdict 齐全的后续同轮五审可用于完成判定。
