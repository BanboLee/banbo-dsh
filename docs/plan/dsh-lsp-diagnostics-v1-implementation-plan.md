...
**未来实现子代理默认使用 `provider: deepseek`、`model: deepseek-v4-flash`；直接人类可以显式覆盖具体 invocation，覆盖事实必须在 routing manifest 中逐次记录。当前 Todo 3 continuation 已由直接人类覆盖为 `provider: runtime-default`（沿用主协调者路由）、`model: gpt-5.6-sol`。自 F1–F5 Round 1 后，直接人类进一步规定所有后续 preflight/acceptance fixer 均使用 `provider: runtime-default`、`model: gpt-5.6-sol`，不得再使用 `deepseek-v4-flash`。恰好五个最终验收 reviewer（F1-F5）也必须使用 `gpt-5.6-sol`。**
...
- Todo6 early-owner repair 在无额外人类覆盖时沿用其 invocation 的 provider/model；F1–F5 Round 1 后的任何 Todo8 integration fixer 或 acceptance fixer 提示必须逐字包含 `provider: runtime-default`、`model: gpt-5.6-sol`。
...
- [x] 8. Wave 4：串行预验收与最终 SHA
  ...
  Recommended task executor category: unspecified-high
  Executor: coordinator；F1–F5 Round 1 后的任何 fixer 使用 provider=runtime-default, model=gpt-5.6-sol

### 固定 fixer contract（Todo 8 preflight 与 F1-F5 后共用）

- 一轮只启动一个 fixer；F1–F5 Round 1 后固定 `provider=runtime-default`、`model=gpt-5.6-sol`，不得再使用 `deepseek-v4-flash`。
...
- 实现 agent 默认使用 `provider=deepseek`、`model=deepseek-v4-flash`；直接人类显式覆盖的 T3-r2 使用 `provider=runtime-default`、`model=gpt-5.6-sol`。F1–F5 Round 1 后所有 integration/acceptance fixer 与 F1-F5 最终 reviewer 均固定 `model=gpt-5.6-sol`（与主协调者相同），provider/route 记录实际启动值，且不得使用 `deepseek-v4-flash`。
