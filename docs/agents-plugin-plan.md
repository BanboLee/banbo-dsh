...
- **T5**（原样重跑 T4）：模型这次选了**前台**（T4 是后台）→ 说明 **F2 生效**。副作用是前台阻塞，用户 5 分钟后那条"停掉它"被排到下一轮，review 已跑完（19.5 分钟）。父 Agent 的回应完全正确："**没有需要停的东西**——审查在你这条消息之前就已经跑完了：那次 `agent_review` 是前台一次性调用，返回即终止，不是可继续的后台 child。刚确认过：当前没有运行中的子 Agent，也没有后台任务。" → **D9 未被触发**。
- **T6**（给一件"无关工作"以诱导后台）：模型推理 "The line count is completely unrelated… **But the count takes seconds; I can just do the count first (fast), then launch the review in foreground.**" → 它**判断正确**（无关工作只要 2 秒，串行优于后台），我的场景设计有缺陷，作废。
- **T7**（明确要求后台 + 4 分钟后叫停）：`agent_review` 后台（child mode `continuable`），父 Agent 结束本轮；新一轮里收到"停掉它"后：
  1. **先 `send_message`**：*"用户要求停止本次审查…请立刻收尾，并基于你已经掌握的证据给我一份阶段性结论…"*（正是 D9 的处方）
  2. 8.6 秒后**读 child 已写下的笔记** `.omo/reviews/delegation-js-review.md`，拿到 3 条 major 发现
  3. **29.1 秒后才 `interrupt_agent`**（`prior_message_to_same_target=1`）
...
- **定义变更后的 fail-open**：guard 由**当前**定义推导，而 toolFilter 是创建时冻结的。一个持久化的带 scope child，如果它的定义后来丢了 `child` 形态、或整个文件被删除（§6.3 支持的删除动作），被 resume 时会**带着 `write`/`edit` 而没有任何 guard**。ABI 会阻止"删除已发布的 child 形态"，但不阻止删除文件。彻底修法有两条：sidecar 是不可变的，可以顺带记录创建时的 scope（升到 version 2）；或者在 resume 时，若冻结的 filter 仍授予 `write`/`edit` 而当前形态没有 scope，就直接拒绝恢复。当前**未实现**，行为由 `write-scope.spec.ts` 的具名用例钉住；
- **listener 里的同步文件 I/O**：`guardResumedChildScope` 每个 subagent 创建都会 `readFileSync` 一次 sidecar。§9.3 要求"同步 listener 中不做文件 I/O"，这里是有意的**例外**：child 的身份只能从 sidecar 得到（header 里只有调用者的 preset），且这个读取有界、吞错、不会 veto 恢复。代价是每个 one-shot child 多一次无用的 syscall（one-shot child 不写 sidecar）；
- **软链接的 scope 根**：scope 根自身的真实路径必须落在工作区的真实路径内，否则拒绝——**这条关掉的是"逃出工作区"**（`<ws>/.banbo-dsh -> /etc`）。但**工作区内部的**软链接仍会放大有效范围：`<ws>/.banbo-dsh -> <ws>` 会让 `.banbo-dsh/plans/x.md` 落到 `<ws>/plans/x.md`，`-> <ws>/src` 则落到 `<ws>/src/plans/x.md`。这**不是**工作区逃逸，且 Planner 自己没有创建软链接的能力（无 `exec`），所以按"声明的是目录、不是工作区内的位置约束"接受，`write-scope.spec.ts` 用**两条具名用例**分别钉住"逃出工作区被拒"与"工作区内放大被接受"；
...
**这个代价与 §5.1 的"小任务自己做"是同向的，不是矛盾**：§5.1 已经按**产出性质**划线——产出是"一条命令的结果"就自己做，产出是"审查结论 / 代码改动 / 计划"才委派。后台化之后，委派一个小任务要多等一轮，于是"小任务自己做"从**允许**变成**经济上更优**，两条规则因此对齐。实测确认了这个效果：一个"跑一下单元测试"的请求，Banbo 自己做完了（162 事件、25 条 fish、**0 次委派**）；而一个"让 executor 在后台跑双 lane 并汇总"的请求，它后台委派并在被唤醒后汇总。**看到 Banbo 自己跑测试不是回归**——除非那个任务的产出是结论或改动。
