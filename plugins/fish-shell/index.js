/**
 * Self-contained fish shell tool for DeepSeek Harness.
 *
 * Registers a `fish` tool that runs commands through `fish -c` instead of
 * bash. This bundle is deliberately dependency-free: it imports only Node
 * built-ins and never any `@deepseek-ai/*` package, so installing it into a
 * profile can never collide with the harness's own copies of those packages
 * (no duplicate-Cordis-instance risk). The trade-off is that the bash
 * executor's advanced mechanics (sandbox, background-job registration,
 * settings-driven budgets, spill files) are not reused; those can be layered
 * in a later version.
 *
 ...
 * the bash tool uses, plus FISH_PAGER.
 */
const ENV_OVERRIDES = {
  NO_COLOR: '1',
  TERM: 'dumb',
...
const OUTPUT = {
  schema: {
    ...
    required: ['kind', 'exitCode', 'signal', 'timedOut', 'aborted'],
    properties: {
